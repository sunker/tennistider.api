...
slotSchema.statics = {
  ...
  getByDate: query => {
    if (query.date) {
      const startDate = new Date(
        new Date(
          new Date(new Date(query.date).setHours(0)).setMinutes(0)
        ).setSeconds(0)
      );
  ...
  countUpcoming: () => {
    return new Promise((resolve, reject) => {
      Slot.count({ date: { $gte: new Date() } }, function(err, count) {
        if (err) {
          reject(err);
        } else {
          resolve(count);
  ...
  getUpcomingByClubs: clubIds => {
    return new Promise((resolve, reject) => {
      Slot.find(
        { date: { $gte: new Date() }, clubId: { $in: clubIds } },
        function(err, slots) {
          if (err) {
            reject(err);
          } else {
